    ...
    try {
      ...
      if (leadsResult.status === 'fulfilled') {
        if (leadsResult.value.error) {
          ...
          leads = (leadsResult.value.data ?? []) as Lead[];
        }
      } else {
        errors.push(
      ...
      if (searchesResult.status === 'fulfilled') {
        ...
        errors.push(
          searchesResult.reason instanceof Error
            ? searchesResult.reason.message
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/25">
          ...
          <div className="flex-1">
            <p className="text-red-300 font-medium text-sm">
              Dashboard data couldn't load completely.
            </p>
            <p className="text-red-400/70 text-xs mt-0.5">{error}</p>
          </div>
          <button
            onClick={load}
            className="btn-primary text-xs py-1.5 flex-shrink-0 inline-flex items-center gap-1.5"
          >
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/25">
          ...
          <div className="flex-1">
            ...
            <p className="text-red-400/70 text-xs mt-0.5">
              Upgrade your plan to continue generating leads, audits, and outreach messages.
            </p>
          </div>
          <button
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-500/10 border border-blue-500/25">
          ...
          <div className="flex-1">
            ...
            <p className="text-blue-400/70 text-xs mt-0.5">
              Choose a plan to unlock more leads, AI audits, and outreach messages.
            </p>
          </div>
          <button
            onClick={() => onNavigate('settings', { tab: 'billing' })}
          ...
          {admin ? (
            <div className="space-y-3">
              <p className="text-slate-400 text-sm">
                Admin accounts have unlimited access to all features.
              </p>
            ...
            <div className="space-y-3">
              <UsageBar
                ...
                limit={limits.leadsLimit}
                icon={Users}
              />
              <UsageBar
                label="AI Audits"
                used={profile?.audits_used_this_month ?? 0}
                limit={limits.auditsLimit}
                icon={BarChart3}
              />
              <UsageBar
                label="Messages"
          ...
          {recentSearches.length === 0 ? (
            ...
            <div className="divide-y divide-slate-800">
              {recentSearches.map((searchItem) => (
                <div key={searchItem.id} className="flex items-start gap-4 px-5 py-3.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-200 font-medium text-sm truncate">
                      {searchItem.niche}
                    </p>
                    <p className="text-slate-500 text-xs truncate">
                      {searchItem.location} · {searchItem.service_offer}
                    </p>
                  </div>
