...
export function WebsitePreviewPage({ token }: Props) {
  ...
  const galleryImages = (preview.images?.gallery || []).filter((image) => isValidImageUrl(image.url));
  const heroImage = isValidImageUrl(preview.images?.hero?.url) ? preview.images?.hero : undefined;
  const markImageFailed = (key: string) => setFailedImages((current) => ({ ...current, [key]: true }));
  const renderImageOrGradient = (key: string, image: typeof heroImage, className: string, gradientClassName = '') => (
    image && !failedImages[key]
      ? <img src={image.url} alt={image.alt || 'Business preview photo'} className={className} onError={() => markImageFailed(key)} loading="lazy" />
      : <div className={`${className} bg-gradient-to-br ${visualBlockStyle} ${gradientClassName}`} />
  );
  ...
  const sectionRenderers: Record<string, () => JSX.Element> = {
    ...
    about: () => <section id="about" key="about" className="mx-auto max-w-7xl px-6 py-16 sm:py-24"><div className="grid gap-10 lg:grid-cols-2 lg:items-center"><div><p className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">About</p><h2 className="mt-3 text-4xl font-black tracking-tight">Built around a better customer experience</h2><p className="mt-6 leading-8 text-slate-600">{preview.about_intro}</p></div><div className={`overflow-hidden rounded-[2rem] bg-gradient-to-br ${visualBlockStyle} p-8 text-white`}><div className="relative h-64">{renderImageOrGradient('about', galleryImages[0], 'absolute inset-0 h-full w-full rounded-[1.5rem] object-cover opacity-80')}<div className="relative p-4"><IndustryIcon className="h-12 w-12" /></div></div><p className="mt-6 text-2xl font-black">{visualTheme.hero_visual_title}</p></div></div></section>,
        ...
        <div className={`relative mx-auto grid max-w-7xl gap-12 px-6 py-20 lg:items-center lg:py-28 ${getHeroLayout(visualTheme)}`}>
          ...
          <div className={`relative rounded-[2.25rem] border p-4 shadow-2xl shadow-slate-900/10 backdrop-blur ${themeClasses.panel}`}>
            <div className={`relative overflow-hidden rounded-[1.75rem] bg-gradient-to-br ${visualGradient} text-white`}>
              {heroImage && !failedImages.hero && <img src={heroImage.url} alt={heroImage.alt || visualTheme.hero_visual_title} className="absolute inset-0 h-full w-full object-cover" onError={() => markImageFailed('hero')} />}
