...
export function LeadDetailPage({ leadId, onBack, onNavigate }: Props) {
  ...
  async function handleAnalyze() {
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke('analyze-lead', {
        body: { lead_id: leadId },
      });
  ...
  async function handleGenerateMessage() {
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke('generate-outreach', {
        body: {
          lead_id: leadId,
          channel: msgChannel,
